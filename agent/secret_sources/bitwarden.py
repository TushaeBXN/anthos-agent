...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `anthos chat -q ...`
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``anthos secrets bitwarden setup``
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `anthos secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `anthos secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `anthos secrets bitwarden setup` to install."
        )
        return result
