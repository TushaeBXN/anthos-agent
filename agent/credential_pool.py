...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``anthos model`` / ``anthos auth`` which
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (e.g. setup wizard, ``anthos auth status``) would land on
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # re-authed via `anthos model` / `anthos auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `anthos model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``anthos auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `anthos auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `anthos auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        ...
        # API-key vs OAuth is a user-visible choice at `anthos setup` ("Claude
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `anthos auth add nous --label <name>`).  Fall back to the
        ...
        # standard `anthos auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `anthos auth remove openai-codex` marks
        ...
        # When the user logs in via ``anthos model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``anthos auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `anthos auth remove <provider> <N>` for an
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `anthos auth` command that confirmed the source is gone).
