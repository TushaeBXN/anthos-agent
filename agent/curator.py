...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `anthos update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``anthos curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``anthos update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``anthos curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `anthos curator run --dry-run` to preview now"
            )
            save_state(state)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "safety and can be restored via `anthos curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Restore any via `anthos curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `anthos curator restore <name>`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``anthos curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `anthos curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `anthos update` only if "
        ...
        # recorded in state so `anthos curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``anthos model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `anthos model` → auxiliary
