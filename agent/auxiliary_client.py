...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``anthos auth
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `anthos setup` or `anthos model` and enter a valid http(s) base URL."
        ) from exc
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``anthos config aux reset``)."""
...
def _resolve_auto(
    ...
    #    scenario where a user switches providers via `anthos model` but the
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `anthos model`."
                )
