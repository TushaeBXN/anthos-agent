...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Anthos starts and finds ``~/.openclaw/``.

    Points users at ``anthos claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``anthos claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Anthos, run "
        "`anthos claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `anthos claw cleanup` (renames it to ~/.openclaw.pre-migration — "
