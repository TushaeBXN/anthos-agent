...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``anthos hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
# ---------------------------------------------------------------------------
# Introspection (used by `anthos hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``anthos hooks test`` and ``anthos hooks doctor``.
    ...
    produce — otherwise scripts tested via ``anthos hooks test`` could
