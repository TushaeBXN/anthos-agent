...
# gateway process and the (separate) ``anthos cron status`` process share it
...
# threshold in `anthos cron status` (hermes_cli/cron.py), so the two never
# drift apart.
...
# =============================================================================
# Ticker heartbeat (liveness signal for `anthos cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``anthos cron status``) never sees a
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `anthos cron status` can tell a thread that is merely *alive and looping*
