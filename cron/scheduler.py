...
# Without this, standalone invocations (e.g. after `anthos update` reloads
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``anthos tools`` config for the ``cron`` platform.
    ...
    try:
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                "default with `anthos model <name>`."
            )
