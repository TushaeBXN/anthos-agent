...
load when listed in ``plugins.enabled`` (via ``anthos plugins enable
observability/langfuse`` or ``anthos tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``anthos tools`` or ~/.hermes/.env):
