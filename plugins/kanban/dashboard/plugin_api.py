...
This means ``anthos dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``anthos kanban
...
def list_diagnostics(
    ...
    AND for ``anthos kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``anthos kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``anthos kanban specify <task_id>``.
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``anthos kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``anthos profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``anthos kanban decompose <task_id>``.
