...
def cmd_sync(args) -> None:
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'anthos honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'anthos honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `anthos update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'anthos honcho setup' again once you have a key.\n")
            return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'anthos honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'anthos honcho setup' to configure.\n")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Hermes' identity in Honcho. Seed with 'anthos honcho identity <file>'.")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'anthos honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'anthos honcho identity <file>' to seed one.")
        print()
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print()
        answer = _prompt("  Run 'anthos honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'anthos honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files:
        ...
        if has_key:
            ...
            print("  Run 'anthos honcho setup' first, then re-run this step.")
    else:
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'anthos honcho setup' first, then seed manually:")
            for f in agent_files:
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'anthos memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``anthos honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``anthos honcho``.
    """
