...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `anthos update` or `--replace` handoffs)
        ...
        message = (
            ...
            "with this token, then restart the gateway with 'anthos gateway restart'."
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``anthos update --gateway``
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``anthos model`` picker via the shared ``group_providers`` fold,
