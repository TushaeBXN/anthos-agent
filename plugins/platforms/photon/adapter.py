...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `anthos photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `anthos setup` / status.
        return False
    return True
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `anthos photon setup`)"
            )
...
def register(ctx) -> None:
    ...
    # gateway-setup hook and the `anthos photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        # Surfaces Photon in `anthos gateway setup` alongside every other
    ...
    # Register CLI subcommands — `anthos photon ...`
