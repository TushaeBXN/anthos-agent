"""
``anthos photon ...`` CLI subcommands — registered by the plugin via
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `anthos photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Anthos Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``anthos photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Anthos Agent" project.
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `anthos photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`anthos photon telemetry on|off`)")
    return 0
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `anthos photon telemetry on` / `anthos photon telemetry off`.")
        return 0
    try:
...
#
# `anthos gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``anthos photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `anthos gateway setup` wizard."""
