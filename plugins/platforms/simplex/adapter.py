...
discoverable and ``anthos setup`` can describe it even when websockets is
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``anthos cron`` running as a
    separate process from ``anthos gateway``). Without this hook,
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``anthos setup gateway`` → SimpleX.
