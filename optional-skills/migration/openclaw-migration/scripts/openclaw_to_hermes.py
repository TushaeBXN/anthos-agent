...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'anthos cron'.",
    },
    "hooks-config": {
...
class Migrator:
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'anthos cron' to recreate jobs manually.")
            else:
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'anthos gateway' to configure.")
    ...
    def generate_migration_notes(self) -> None:
        ...
        notes.extend([
            ...
            "",
            "**Strongly recommended:** Run `anthos claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `anthos claw cleanup` to fix it.",
            "",
            "## Hermes-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `anthos claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `anthos setup` to configure any remaining settings",
            "- Run `anthos mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `anthos cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `anthos cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `anthos gateway install` if you need the gateway service",
