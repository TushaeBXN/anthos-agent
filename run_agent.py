...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``anthos -c`` / ``--resume`` (relaunch
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"AnthosAgent/{_HERMES_VERSION}",
    }
...
class AIAgent:
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``anthos chat -q``. In that mode,
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        if _dev:
            ...
            # `anthos logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``anthos auth add xai-oauth``) whose tokens belong to
