...
def looks_like_gateway_command_line(command: str | None) -> bool:
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return True  # bare `anthos gateway` defaults to `run`
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `anthos status` show per-profile
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``anthos gateway stop`` on Windows would be misclassified as an
