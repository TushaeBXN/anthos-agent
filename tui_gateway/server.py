...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `anthos --tui` keeps real _stdio.
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `anthos --tui`, so it must NOT count as dead here (doing
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    # The desktop app and `anthos --tui` both land here. See
...
def _(rid, params: dict) -> dict:
    ...
    # ``anthos --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``anthos gateway`` process, whose
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`anthos setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`anthos gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`anthos sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`anthos config edit` needs $EDITOR in a real terminal"
    return None
    ...
    try:
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `anthos model` to configure",
            )
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``anthos plugins`` / the dashboard, so the three surfaces
