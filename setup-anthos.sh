...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/anthos-uv-install.$$.log")"
        _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/anthos-uv-installer.$$.sh")"
...
echo -e "${CYAN}→${NC} Setting up anthos command..."

HERMES_BIN="$SCRIPT_DIR/venv/bin/anthos"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$HERMES_BIN" "$COMMAND_LINK_DIR/hermes"
echo -e "${GREEN}✓${NC} Symlinked anthos → $COMMAND_LINK_DISPLAY_DIR/hermes"
