...
Backend is selected during ``anthos tools`` setup (web.backend in config.yaml).
...
def _env_value(name: str) -> str:
    ...
    through Hermes' config/.env layer (``anthos config set``, ``anthos tools``)
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``anthos tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `anthos tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `anthos tools` to set one up."
                ),
            }
        else:
            logger.info(
