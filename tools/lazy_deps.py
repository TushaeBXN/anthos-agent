...
remediation hint pointing at ``anthos tools`` or the manual pip command.
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``anthos update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def active_features() -> list[str]:
    ...
    Used by ``anthos update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``anthos update``. Never raises; lazy-install failures
