...
orchestrator work. A normal ``anthos chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``anthos kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``anthos kanban complete …``
...
Humans continue to use the CLI (``anthos kanban …``), the dashboard
(``anthos dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``anthos chat`` without the kanban toolset see zero
