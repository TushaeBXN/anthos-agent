...
def read_suppressed_names() -> Set[str]:
    ...
    what makes pruning a built-in durable: without it, ``anthos update`` would
...
def list_archived_skill_names() -> List[str]:
    ...
    so the directory name is the skill name. Used by ``anthos curator
    list-archived`` to help users pass a name to ``anthos curator restore``.
    """
