...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print_info("Run 'anthos setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'anthos setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'anthos setup' to configure"))
    ...
    # Video generation — opt-in via `anthos tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'anthos setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'anthos setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'anthos setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'anthos setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('anthos setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('anthos setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('anthos setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('anthos setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('anthos setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('anthos config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('anthos config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('anthos config set <key> <value>', Colors.GREEN)}")
    ...
    print(f"   {color('anthos gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('anthos doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `anthos tools` and `anthos setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``anthos model``)
    ...
    provider added to ``anthos model`` is automatically available here.
    ...
    # on demand via `anthos auth add`, `anthos setup` vision, and
    # `anthos setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``anthos model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'anthos setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `anthos setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `anthos setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `anthos setup agent` later to customize.")
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'anthos setup gateway' later to configure.")
        return
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `anthos setup tools` and `anthos tools` use the same flow:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``anthos setup --portal`` and ``anthos portal``. This is the
    ...
    ``anthos setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``anthos model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``anthos portal`` always offers a model picker,
    ...
    # setup calls, so `anthos portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `anthos portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `anthos portal`: %s", exc)
        print()
    ...
    print_info("  Run `anthos portal info` to inspect routing.")
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'anthos setup model|terminal|")
    ...
    print()
    print_info("You can edit these files directly or use 'anthos config edit'")
    ...
    # Tune later with `anthos setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``anthos setup <section>``
    or switch providers with ``anthos model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'anthos setup gateway'",
        ],
        0,
    )
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``anthos tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``anthos setup agent`` /
    ``anthos config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `anthos update` runs don't re-inject them.
        try:
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`anthos update` runs from re-injecting them. Opt back in any")
            print_info("time with `anthos skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `anthos tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `anthos plugins list` / `anthos plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `anthos plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `anthos mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `anthos mcp add`.")
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'anthos setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'anthos setup gateway'.")
