...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # full device-code flow. This makes `anthos --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``anthos auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`anthos auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `anthos auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `anthos auth` is called bare."""
