"""MCP picker — interactive `anthos mcp picker` (also the default `anthos mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``anthos mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `anthos plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `anthos mcp catalog`."""
    if not rows:
        print()
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Hermes — run `anthos update` "
...
def show_catalog() -> None:
    """`anthos mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`anthos mcp picker` (and default `anthos mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`anthos mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `anthos mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
