...
Add, remove, or reorder entries here — both `anthos setup` and
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `anthos model`
...
#   tui_desc    — longer description for the `anthos model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `anthos model` TUI
...
# slug as a top-level row in the interactive `anthos model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``anthos model``, the
...
# path. The *interactive* default (GUI onboarding / ``anthos model``) uses the
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``anthos auth add openai-codex`` without
    ``anthos model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``anthos model`` picker offers first. For metered aggregators
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``anthos model``, ``/model``, etc.).
    """
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``anthos auth add copilot`` and by ``_seed_from_env``
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``anthos model`` shows.
