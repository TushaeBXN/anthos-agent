"""Implementation of the ``anthos bundles`` CLI subcommand.
...
def _console() -> Console:
    # Bind to stderr so piping `anthos bundles list | grep …` doesn't
...
def register_cli(subparser) -> None:
    """Build the ``anthos bundles`` argparse tree.
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `anthos bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``anthos bundles <subcommand>`` to the right handler."""
