"""
Configuration management for Anthos Agent.
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``anthos config set`` (which rewrites
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``anthos logs``
...
#   on every plugin install / ``anthos update``.
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `anthos plugins enable
    # observability/langfuse` or `anthos tools → Langfuse`); credentials gate
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade anthos-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    ``docker`` and ``anthos update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/anthos-agent`` image bakes a ``docker``
    ...
    #    container, and honouring it wrongly blocks ``anthos update``.
    try:
        method = (
...
def is_uv_tool_install() -> bool:
    ...
    ``uv tool install anthos-agent`` places the install at
    ``.../uv/tools/anthos-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``anthos-agent`` happens to be uv-tool-installed on the machine while
    the *active* Hermes is a regular pip/venv install, causing
    ``anthos update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/anthos-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade anthos-agent"
    if method == "docker":
        return "docker pull nousresearch/anthos-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade anthos-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade anthos-agent"
        return "pip install --upgrade anthos-agent"
...
# Long-form text for ``anthos update`` / ``--check`` when running inside the
...
✗ ``anthos update`` doesn't apply inside the Docker container.

Anthos Agent runs as a published image (nousresearch/anthos-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/anthos-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate anthos-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/anthos-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/anthos-agent/tags
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``anthos update`` inside Docker.
...
def format_managed_message(action: str = "modify this Hermes installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            ...
            "Edit services.anthos-agent.settings in your configuration.nix and run:\n"
...
def get_container_exec_info() -> Optional[dict]:
    ...
    container_name = info.get("container_name", "anthos-agent")
...
DEFAULT_CONFIG = {
    ...
    # Opt in via ``anthos chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
    ...
    # kanban dashboard (see ``anthos dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``anthos kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``anthos kanban decompose`` and the kanban
        ...
        # ``anthos profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `anthos model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `hermes` (and `anthos chat`) launches by default:
        ...
        "interface": "cli",
        # When true, `anthos --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `anthos -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERMES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `anthos --tui` drops a one-time hint
    ...
    # cron daemon.
    #
    # See `anthos curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `anthos curator run --consolidate` overrides
        ...
        # normally restored on every `anthos update`, so pruning them only
        ...
        # user can roll back with `anthos curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "✦ *Anthos Agent*" header.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `anthos doctor --ack
    ...
    # todos to ready, and fires `anthos -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # decomposition is manual via `anthos kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     anthos-agent's own python (sys.executable). Maximum isolation
    ...
    # update model picker lists without shipping a anthos-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://anthos-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `anthos model` invocation; network failures
    ...
    "sessions": {
        ...
        # default of ``anthos sessions prune``.
        "retention_days": 90,
    ...
    # ``anthos update`` behaviour.
    "updates": {
        # Run a full ``anthos backup``-style zip of HERMES_HOME before every
        # ``anthos update``.  Backups land in ``<HERMES_HOME>/backups/`` and
        # can be restored with ``anthos import <path>``.  Defaults to true
        # after the #48200 incident: a ``anthos update --yes`` run that
        ...
        "backup_keep": 5,
        # What `anthos update` does with uncommitted local changes to the
    ...
    # `anthos tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    "secrets": {
        "bitwarden": {
            ...
            # `anthos secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'anthos model' for endpoint-specific config)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'anthos-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `anthos update`.  Skill-config prompting is a
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'anthos setup' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'anthos doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `anthos plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `anthos curator status` has no
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``anthos config set``
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`anthos tools`, `anthos setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `anthos tools`
...
def show_config():
    ...
    print(color("│              ✦ Anthos Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                ...
                f"(run 'anthos doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `anthos config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `anthos config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if missing_config:
            ...
            print("    Run 'anthos config migrate' to add them")
...
# ``_inject_profile_env_vars`` surfaces them in ``anthos config`` UI so users
