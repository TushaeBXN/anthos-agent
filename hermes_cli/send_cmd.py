"""CLI subcommand: ``anthos send`` — pipe text from shell scripts to any
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `anthos gateway setup`, or run the gateway once so")
...
def _load_hermes_env() -> None:
    ...
    startup that ``anthos send`` must replicate when invoked standalone:
    ...
    2. Bridge top-level simple values from ``~/.hermes/config.yaml`` into
       ...
       via ``anthos config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``anthos send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Import lazily so `anthos send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `anthos send --list telegram`.",
    )
