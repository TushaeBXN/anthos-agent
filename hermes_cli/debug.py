"""``anthos debug`` debug tools for Hermes Agent.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``anthos debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``anthos debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `anthos debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``anthos debug share`` invocation added ~20 MB of resident Python
    ...
    ``anthos debug share`` also runs an opportunistic sweep as a fallback
...
def _capture_dump() -> str:
    """Run ``anthos dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``anthos dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``anthos debug share`` (CLI) and the
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `anthos debug share --local` to print the report instead.\n")
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        ...
        print("  Deletes paste.rs pastes uploaded by 'anthos debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``anthos debug`` call.
    ...
    # best-effort — any failure is swallowed so ``anthos debug`` stays
