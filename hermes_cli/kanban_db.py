...
``anthos -p <profile>`` joins the same board as the dispatcher that
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``anthos kanban boards
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``anthos kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``anthos kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``anthos kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``anthos kanban log`` unambiguous even when multiple
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``anthos gateway run --replace`` /
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    # holder (e.g. an external `anthos kanban list` mid-integrity-probe) block
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``anthos kanban init`` and the
...
def create_task(
    ...
    *,
    ...
    each name to ``anthos --skills ...``. Use this to pin a task to a
    ...
    # `anthos --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``anthos kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `anthos kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``anthos kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``anthos kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``anthos kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``anthos kanban complete <id>``) works without requiring
...
#      via ``anthos kanban show <id>`` and the dashboard.
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``anthos kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``anthos kanban
...
def _dispatch_once_locked(
    ...
    *,
    ...
    for row in ready_rows:
        ...
        # `_default_spawn` invokes ``anthos -p <assignee>`` which fails
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `anthos kanban tail` — without
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``anthos -p <profile> chat -q ...`` subprocess.
    ...
    # env, and when the child process starts `anthos -p <name>` the
    ...
    # that even when the worker activates a profile (`anthos -p <name>`
    ...
    # `anthos -p <assignee>` activates the profile, but the env var is
    ...
    # `anthos kanban log` on a specific board reads its own file and
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``anthos kanban daemon`` is systemd-friendly.
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``anthos kanban assignees`` for the terminal.
