"""``anthos portal`` — the human-readable entry point for Nous Portal.

Running ``anthos portal`` with no subcommand performs the one-shot Portal
...
``anthos auth add nous --type oauth`` (which still works), is identical to
``anthos setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``anthos auth`` or ``anthos tools``. It's the onboarding + discovery
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `anthos model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `anthos portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `anthos auth add nous --type
    oauth`. It reuses the exact wiring behind `anthos setup --portal` (which in
...
def portal_command(args) -> int:
    """Top-level dispatch for `anthos portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `anthos portal` is the
        # human-readable alias for `anthos auth add nous --type oauth` /
        # `anthos setup --portal`.
    ...
    print("Run `anthos portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `anthos portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `anthos portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `anthos auth add "
            "nous --type oauth`, identical to `anthos setup --portal`). "
