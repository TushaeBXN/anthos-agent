...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``anthos desktop`` / ``anthos gui``)
...
``anthos uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def source_built_gui_artifacts(hermes_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``anthos desktop`` inside the checkout.
