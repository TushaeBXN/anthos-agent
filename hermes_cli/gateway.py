...
# when hermes_constants is cached from a pre-update version during `anthos update`.
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``anthos gateway status`` from falsely counting the ``hermes`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``anthos gateway status``) is never mistaken for a running
    # gateway.  See #13242.
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``anthos update``
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``anthos update`` users
    # with no running gateway until they re-invoke ``anthos gateway``
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `anthos gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``anthos gateway status`` output so users with
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``anthos gateway run`` must remain interruptible from
...
SERVICE_DESCRIPTION = "Anthos Agent Gateway - Messaging Platform Integration"
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``anthos update`` later relocates/removes — the path
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``anthos gateway restart``
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `anthos gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `anthos gateway status` or logs for final state."
            )
            return
...
# process invocation (each ``anthos gateway start/stop/status`` call).
...
def launchd_stop():
    ...
    # `anthos gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``anthos gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``anthos gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``anthos gateway run`` can burn memory/CPU just to fail with "already
    ...
    print("  Use 'anthos gateway restart' to replace it,")
    print("  or 'anthos gateway stop' first.")
    print("  Or use 'anthos gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `anthos gateway run` still
    ...
    # /restart) rather than through `anthos gateway restart` which already
    ...
    # `anthos gateway start/restart` — leaving the gateway vulnerable to
...
def _all_platforms() -> list[dict]:
    ...
    ``anthos setup gateway`` without needing the gateway to be running.
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'anthos pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Skipped — configure later with 'anthos gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `anthos gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `anthos pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `anthos pairing approve`."
        )
    elif access_idx == 1:
    ...
    if any_configured:
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                print_info(
                    "  For persistence:   tmux new -s hermes 'anthos gateway run'"
                )
                print_info(
        ...
        print()
        print_info("No platforms configured. Run 'anthos gateway setup' when ready.")
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``anthos gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``anthos gateway install|uninstall|start|stop|restart``
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s hermes 'anthos gateway run'"
                )
                print()
            ...
            print(
                "  tmux new -s hermes 'anthos gateway run'         # persistent via tmux"
            )
            print(
        ...
        # slot — start them individually via `anthos -p <name> gateway
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `anthos gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `anthos gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
