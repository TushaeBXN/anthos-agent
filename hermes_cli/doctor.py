...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    other, leaving ``anthos --version`` reporting a stale version while
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            "Re-sync version files (e.g. run 'anthos update', or set "
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `anthos profile create <name>`")
        return
...
def run_doctor(args):
    ...
    # Handle `anthos doctor --ack <id>` as a fast path. Persist the ack and
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`anthos doctor --ack {hit.advisory.id}`."
                )
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'anthos setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'anthos setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'anthos setup' to create one")
                issues.append("Run 'anthos setup' to create .env")
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'anthos config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.hermes/.env or run 'anthos setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'anthos setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'anthos config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'anthos setup' to migrate config")
                else:
                    issues.append("Run 'anthos doctor --fix' or 'anthos setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'anthos doctor --fix'")
        except Exception:
            pass
        ...
        try:
            ...
            if drift:
                check_warn(
                    ...
                    "(stale ghost from an earlier `anthos setup` run)",
                )
                if should_fix:
    ...
    if soul_path.exists():
        ...
        if should_fix:
            ...
            soul_path.write_text(
                ...
                "You are Anthos, a helpful AI assistant.\n",
                encoding="utf-8",
            )
        ...
        try:
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'anthos doctor --fix' "
                        "(or 'anthos sessions repair') to recover hidden sessions"
                    )
            else:
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'anthos doctor --fix' to checkpoint")
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    ...
                    if should_fix:
                        ...
                        issues.append(f"Broken symlink at {_cmd_link_display}/hermes — run 'anthos doctor --fix'")
                ...
                if should_fix:
                    ...
                    issues.append(f"Missing {_cmd_link_display}/hermes symlink — run 'anthos doctor --fix'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `anthos doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'anthos config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``anthos doctor``.
    ...
    # never the legitimate source for `anthos doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'anthos setup' to configure missing API keys for full tool access")
    except Exception as e:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                _fail_and_issue(
                    ...
                    "No Honcho API key — run 'anthos memory setup'",
                    issues,
                )
            else:
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'anthos doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
