"""CLI handlers for ``anthos migrate ...``.

Currently exposes only ``anthos migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``anthos migrate <subtype>``."""
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `anthos migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `anthos doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
