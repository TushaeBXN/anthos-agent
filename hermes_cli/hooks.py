...
def hooks_command(args) -> None:
    """Entry point for ``anthos hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: hermes hooks {list|test|revoke|doctor}")
        print("Run 'anthos hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        ...
        print("See `anthos hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `anthos hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `anthos hooks test` and `anthos hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `anthos hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `anthos hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `anthos hooks doctor`.")
