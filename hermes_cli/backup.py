...
`anthos backup` creates a zip archive of the entire ~/.hermes/ directory
(excluding the hermes-agent repo and transient files).

`anthos import` restores from a backup zip, overlaying onto the current
HERMES_HOME root.
"""
...
# File names that ``anthos import`` must never overwrite, matched by basename so
...
# — `anthos update` snapshots this set before pulling so approved-user lists
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``anthos update``.
...
def create_pre_update_backup(
    ...
    (``anthos update``) should continue even if the backup fails.
    """
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `anthos claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    # Reuses the shared backups/ directory so `anthos import` and the
