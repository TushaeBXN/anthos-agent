...
``anthos gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``anthos gateway start/stop/restart``).
    """
    ...
    # `anthos gateway start/stop/restart` fell through to host code that spawns
...
# active (set via the ``anthos -p <profile>`` flag before the call). This
...
#
# Per-profile gateways are registered dynamically when `anthos profile create`
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``anthos -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`anthos profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          ...
          4. Drops to the hermes user and exec's
             ``anthos -p <profile> gateway run`` (or just ``hermes
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``anthos gateway
        ...
        # (a stray ``anthos gateway run`` from a shell, an agent action, or
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``anthos logs`` reads and what
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``anthos gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``anthos -p <profile> gateway start``.
