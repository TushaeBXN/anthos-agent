...
try:
    ...
    # running `anthos dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``anthos gateway run`` — but the
    desktop app spawns a ``anthos dashboard`` backend, not a gateway, so a cron
...
app = FastAPI(title="Anthos Agent", version=__version__, lifespan=_lifespan)
...
class ModelAssignment(BaseModel):
    ...
    # ``anthos model`` custom flow collects. Honored only on the main slot for
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``anthos update``.
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # and which providers are registered so ``anthos status`` and the
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``anthos <subcommand>`` detached and record the Popen handle.
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``anthos gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``anthos gateway restart`` children race each other on the
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``anthos gateway restart`` in the background."""
    try:
...
async def update_hermes():
    """Kick off ``anthos update`` in the background."""
...
async def check_hermes_update(force: bool = False):
    ...
    ``POST /api/hermes/update`` actually runs ``anthos update``.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `anthos model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `anthos model` does so GUI onboarding lands on a
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # endpoint, mirroring the ``anthos model`` custom flow
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``anthos model`` universe). This is what lets the desktop Keys tab
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `anthos model` provider — would be
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict) -> dict:
        ...
        return {
            ...
            # CLI `anthos model` picker uses (not desktop-only prefix guesses).
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Anthos Agent").strip() or "Anthos Agent"
...
# the canonical ``anthos auth add <provider>`` command so the dashboard
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``anthos model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `anthos model` universe (zero-edit for new plugins).
    try:
        from hermes_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `anthos model` picker; _OAUTH_OVERRIDES supplies per-provider
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``anthos auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
# the tokens exactly like `anthos auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `anthos auth add xai-oauth`'s credential-pool insert.
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `anthos sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `anthos sessions prune`)."""
...
# servers managed here show up under `anthos mcp list` and vice versa.  Secrets
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `anthos mcp catalog` / `anthos mcp install` read.  ``profile`` scopes
...
# spawn the real `anthos gateway <verb>` so behaviour matches the CLI exactly.
...
# `anthos memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `anthos memory setup` to configure a new one.",
            )
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `anthos import`. The spawned action runs with
...
# Human-readable labels for each hub source id (matches `anthos skills search`
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`anthos -p <name> skills install`)
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``anthos profile use`` —
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``anthos profile use``).
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``anthos profile describe
    <name> --auto``.
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``anthos tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``anthos tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``anthos tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    the same store ``anthos tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``anthos tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``anthos -p <profile> tools post-setup``.
...
#
# The endpoint spawns the same ``anthos --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``anthos --tui`` would run.  Tests monkeypatch this
    ...
    — the same propagation ``anthos -p <name>`` performs. The in-process
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `anthos dashboard register` (Nous Portal) or "
