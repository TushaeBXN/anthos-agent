...
# When the user upgrades code via ``git pull`` (or ``anthos update``
...
# ``anthos update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``anthos tools`` or
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`anthos --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    ...
    print(f"Anthos Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``anthos --version`` before config/logging imports on Termux."""
...
def _require_tty(command_name: str) -> None:
    ...
    if not sys.stdin.isatty():
        print(
            f"Error: 'anthos {command_name}' requires an interactive terminal.\n"
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `anthos mcp add ... --args <command argv>`.
    ...
    # after the subcommand (`anthos chat -p coder`), so keep scanning broadly.
    ...
    # `anthos profile use` and the gateway should honour that choice.
    # See issue #22502.
    ...
    # reserved ``gateway-default`` slot runs bare ``anthos gateway run`` to mean
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``anthos update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        ...
        "This usually means `anthos update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Hermes checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `anthos --tui`\n"
        "If the checkout is still inconsistent, run `anthos update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _launch_tui(
    ...
    # in the user's shell would otherwise make a plain `anthos --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `anthos update` so
...
def _pin_kanban_board_env() -> None:
    ...
    (``anthos kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``anthos kanban boards switch`` from another session can flip the file
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'anthos sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'anthos doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'anthos setup' at any time to configure.")
        sys.exit(1)
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `anthos gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `anthos gateway` skips it cleanly instead of paying a 30s
        ...
        print("⚠ Pairing may not have completed. Run 'anthos whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``anthos whatsapp`` (the Baileys bridge wizard) — the
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``anthos model``) and the setup wizard
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'anthos model' for "
                "available providers, or run 'anthos doctor' to diagnose config "
                "issues."
            )
...
# `anthos model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `anthos model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``anthos model`` flow, then route aux tasks to them here.
    """
    from hermes_cli.config import load_config
...
# fast paths like `anthos --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``anthos setup`` / ``anthos model``.
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `anthos setup` to configure {pconfig.name} again."
        )
        return "", True
...
def cmd_slack(args):
    ...
    Dispatches ``anthos slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            ...
            "\n"
            "Run `anthos slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1
...
def cmd_security(args):
    """Dispatch `anthos security <subcmd>`."""
...
# even run ``anthos update`` again to roll forward. The post-pull syntax
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``anthos update --gateway`` so interactive prompts (stash restore,
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``anthos update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``anthos web``).
...
#   - ``anthos update`` can unconditionally call ``anthos desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``anthos desktop`` (interactive launch) skips the build when the
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``anthos update`` that pulled new source but hasn't built yet).
    """
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``anthos dashboard`` processes other than ourselves.

    ``anthos dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``anthos update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``anthos dashboard`` as
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `anthos update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``anthos update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``anthos update`` invocations skip the block until a newer
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``anthos dashboard`` processes.

    Called at the end of ``anthos update`` (default ``reason``) and also
    from ``anthos dashboard --stop`` (which overrides ``reason``).  The
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `anthos update "
            f"--branch {branch}`, or update against main with `anthos update`."
        )
        sys.exit(1)
    zip_url = (
...
# =========================================================================
# Fork detection and upstream management for `anthos update`
# =========================================================================
...
def _invalidate_update_cache():
    ...
    ``anthos update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumb dropped right before ``anthos update`` mutates the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``anthos update`` left half-done.
    ...
    protocol stream (``anthos acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `anthos update` was interrupted mid-install — "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``anthos update`` is still progressing even if pip/uv itself is silent.
    """
...
def _detect_concurrent_hermes_instances(
    ...
    so during ``anthos update`` the user-invoked process and the desktop's
    ...
    chain are excluded so the running ``anthos update`` invocation never
    ...
    # file lock) is the parent. Without excluding it, every ``anthos update``
...
def _format_concurrent_instances_message(
    ...
    lines.append("  stop the gateway (`anthos gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `anthos update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    when ``anthos update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            ...
            "gateway, or pause AV scanning, then re-run `anthos update`."
        )
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``anthos update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `anthos update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``anthos update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``anthos update``, not the venv we just wrote
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `anthos update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `anthos update --force` after closing other hermes processes."
        )
        return
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `anthos update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``anthos update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``anthos update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``anthos update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``anthos update --gateway``) the update is already
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``anthos update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``anthos update`` (apply
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``anthos update`` without requiring a reinstall.  The
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories.  The ``--backup`` flag on ``anthos update``
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``anthos update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `anthos update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # every user who ran ``anthos update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``anthos update`` again later once a fix lands.")
                    else:
        ...
        # build.  ``anthos desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``anthos desktop`` shouldn't be forced into a full
        # Electron build by ``anthos update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        ...
        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `anthos desktop` to retry)")
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'anthos config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                print()
                print("Skipped. Run 'anthos config migrate' later to configure.")
        else:
        ...
        # check `anthos curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``anthos update`` gives users a predictable
        ...
        # When running as ``anthos update --gateway`` (spawned by the gateway's
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `anthos update` restart it automatically, allow\n"
                            ...
                            # path in `anthos gateway restart`
        ...
        # every `anthos update` surfaces the issue until the user migrates.
        try:
            from hermes_cli.gateway import (
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``anthos -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `anthos profile` — show current profile status
    ...
    if action == "list":
        ...
        if dist_name:
            ...
            print(f"  (run `anthos profile info {name}` for full manifest)")
        if alias_name:
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `anthos profile install` can be updated."
                )
                sys.exit(1)
...
def _report_dashboard_status() -> int:
    """Print ``anthos dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``anthos dashboard --status``
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No hermes dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `anthos update`.
...
# so that in ``anthos -m gpt5 chat``, ``gpt5`` is correctly skipped as a
...
def _first_positional_argv() -> str | None:
    ...
    ``anthos -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `anthos --tui` is the hot path on phones. The full parser setup imports
...
def main():
    ...
    # ``anthos update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # Self-heal a venv left half-built by an interrupted ``anthos update``
    ...
    # ``anthos skills install update``) merely defers recovery one launch;
    # under-matching (missing ``anthos -p work update``) would race a recovery
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `anthos model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `anthos whatsapp` which sets up "
    ...
    # subcommand — ``anthos --help``, ``anthos version``, ``anthos logs``,
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `anthos computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `anthos tools` runs when you first\n"
    ...
    # run `anthos desktop` from a terminal, so the canonical name needs
    ...
    # e.g. ``anthos -c Pokemon Agent Dev`` → ``anthos -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'anthos -c model' where 'model' is consumed as the
