"""``anthos plugins`` CLI subcommand — install, update, remove, and list plugins.
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `anthos plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `anthos plugins enable {installed_name}` to activate.[/dim]",
        )
...
def _resolve_plugin_key(name: str) -> Optional[str]:
    ...
    This is the single normalization point so ``anthos plugins enable`` /
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``anthos plugins list`` CLI filters."""
    filtered = entries
