...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("serve", "Start the local Anthos model server", "Configuration",
               args_hint="[--port N] [--device cpu|cuda]"),
    ...
    CommandDef("version", "Show Anthos Agent version", "Info", aliases=("v",)),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``anthos skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``anthos skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``anthos skills list`` and
