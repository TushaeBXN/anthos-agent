...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``anthos gateway run`` processes
...
#   4. On the ``anthos dashboard`` + gateway paths, drops files into
...
def _uninstall_profile(profile) -> None:
    ...
    We shell out to ``anthos -p <name> gateway stop|uninstall`` because
...
def run_gui_uninstall(args):
    ...
    Mirrors ``anthos uninstall --gui``. Removes the desktop app's built
    ...
    print("or 'anthos uninstall' to remove the agent too.")
    print()
