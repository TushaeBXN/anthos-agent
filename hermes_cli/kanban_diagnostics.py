...
class DiagnosticAction:
    ...
    * ``cli_hint`` — print/copy a shell command (e.g.
      ``anthos -p <profile> auth``). No HTTP side effect.
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    With auto-decompose off, the user must run ``anthos kanban specify``,
    ...
    if auto_decompose:
        ...
        detail_path = (
            ...
            "`anthos kanban specify`, which uses auxiliary.triage_specifier."
        )
