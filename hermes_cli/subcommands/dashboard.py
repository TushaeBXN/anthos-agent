"""``anthos dashboard`` subcommand parser.
...
def build_dashboard_parser(
    ...
    # for `anthos dashboard` cmdlines and SIGTERM them directly — the same
    # path `anthos update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
    ...
    # backend as `anthos dashboard --no-open --tui --host ... --port ...`. The
    ...
    # `anthos dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.hermes/.env.
    # Nested subparser so bare `anthos dashboard` keeps launching the server
