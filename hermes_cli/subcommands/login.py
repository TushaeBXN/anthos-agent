"""``anthos login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``anthos login`` was removed in favor of ``anthos auth`` / ``anthos model``
    ...
    old scripts/aliases invoking ``anthos login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``anthos --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``anthos login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``anthos model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `anthos auth` to manage credentials, "
            "`anthos model` to select a provider, or `anthos setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `anthos model`",
    )
    login_parser.add_argument(
