...
The provider list shown by ``anthos model`` (CLI/TUI) and the desktop Settings
...
the same universe ``anthos model`` renders (``CANONICAL_PROVIDERS``), joining:
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``anthos model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `anthos model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``anthos model`` universe.
