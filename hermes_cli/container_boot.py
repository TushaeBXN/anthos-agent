...
def reconcile_profile_gateways(
    *,
    ...
    is what ``anthos gateway start`` (no ``-p``) targets. Without it,
    bare ``anthos gateway start`` inside the container would land on
    ...
    # ``anthos gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `anthos profile create` (config.yaml
            # is not — that comes later via `anthos setup`). Use it as the
            ...
            # via ``anthos -p default-named gateway start`` if they
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    ...
    the wrapper re-execs ``anthos <subcommand>``. Peel ``init`` →
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``anthos dashboard ...``) never spawns or
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `anthos -p <profile> gateway start`
