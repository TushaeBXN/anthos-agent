...
TIPS = [
    ...
    "Profile names become shell commands — 'anthos profile create coder' creates the 'coder' command.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'anthos profile create ls' would be rejected.",
    ...
    # --- Curator ---
    'anthos curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "hermes curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'anthos curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'anthos auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'anthos dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'anthos dashboard embeds the full Hermes TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'anthos -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'anthos chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'anthos chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'anthos chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "hermes chat --source tool tags programmatic chats so they don't clutter hermes sessions list.",
    'anthos dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'anthos sessions rename <ID> "new title" renames any past session; hermes sessions delete <ID> removes one.',
    'anthos import restores a session export or profile archive produced by sessions export or profile export.',
    'anthos fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'anthos pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'anthos setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'anthos status --deep runs the full health sweep across every component; plain hermes status is the quick view.',
