...
#   backups             — `anthos backup` archives
...
# Marker file written by `anthos profile create --no-skills`.  When present in
...
# `anthos skills install` or drop SKILL.md files into the profile's skills/.
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    try:
        ...
        # not be migrated. The next `anthos doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``anthos -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``anthos profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``anthos profile list`` for an unrelated profile.
    """
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``anthos update`` skips re-seeding this profile's
    ...
    # Keys pages, `anthos -p <name> auth add`) had no file until first
    ...
    # Write the opt-out marker so seed_profile_skills() and `anthos update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`anthos profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `anthos update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `anthos profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `anthos -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``anthos profile create
