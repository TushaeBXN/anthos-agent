...
Powers both:
  - `anthos skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `anthos skills install`.
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    # into `anthos skills install` fail.
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # The identifier is what you pass to `anthos skills install`. Browse used
    ...
    c.print("[dim]Tip: 'anthos skills inspect <identifier>' to preview, "
            "'anthos skills install <identifier>' to install, "
            "'anthos skills search <query>' to search deeper[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                "[bold bright_cyan]This is an official optional skill maintained by Brian Tushae Thomas.[/]\n\n"
...
def do_list(source_filter: str = "all",
    ...
    config — ``anthos -p <profile> skills list`` reads that profile's
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `anthos update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `anthos update`):[/]")
...
def skills_command(args) -> None:
    """Router for `anthos skills <subcommand>` — called from hermes_cli/main.py."""
    ...
    if action == "browse":
        ...
        _console.print("Run 'anthos skills <command> --help' for details.\n")
