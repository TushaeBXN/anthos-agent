...
`anthos tools` and `anthos setup tools` both enter this module.
...
# who want it opt in via `anthos tools` → Video Generation, which walks
...
# `anthos tools` → X (Twitter) Search setup walks users through credential
...
# Platform-scoped toolsets: only appear in the `anthos tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``anthos tools`` → "reconfigure existing" would
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``anthos tools`` checklist actually offers
    for ``platform``.
    ...
    so ``anthos tools`` never claims to add or remove a toolset the user was
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``anthos update`` and
      by ``anthos computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `anthos update` calls this for every
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Run the full `anthos auth spotify` flow — if the user has no
        ...
        try:
            ...
            # toolset enable; they can retry with `anthos auth spotify`.
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `anthos auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``anthos tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``anthos tools post-setup <key>`` — non-interactive post-setup runner.
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # through ``anthos tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `anthos tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `anthos tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `anthos tools`
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `anthos tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `anthos tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `anthos model`).
    if managed_feature:
        from hermes_cli.nous_subscription import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `anthos tools` and `anthos setup tools`.
