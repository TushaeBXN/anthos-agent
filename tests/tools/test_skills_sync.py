...
class TestNoBundledSkillsOptOut:
    ...
    This is what `anthos profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.hermes) rely on so bundled
    skills are never seeded at install time NOR re-injected by `anthos update`.
    """
...
class TestOptOutToggleAndRemove:
    """`anthos skills opt-out/opt-in` core: marker toggle + safe removal."""
