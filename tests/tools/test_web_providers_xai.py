...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``anthos tools`` repaint and at
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `anthos tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``anthos tools``,
...
class TestXAIProviderOAuthPath:
    ...
    runtime resolver (``anthos auth`` sign-in) rather than an env-var key.
