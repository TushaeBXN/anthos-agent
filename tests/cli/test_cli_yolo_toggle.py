...
prompt. Only ``anthos --yolo`` (process-start env), ``HERMES_YOLO_MODE=1``,
and ``anthos config set approvals.mode off`` actually bypassed.
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``anthos --yolo`` sets ``HERMES_YOLO_MODE`` before tool imports, so
