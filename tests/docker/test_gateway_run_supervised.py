...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``anthos gateway run`` invocation
        (not ``sleep infinity``).
...
def test_supervised_gateway_does_not_recurse(
    ...
    supervised ``anthos gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``anthos gateway
    ...
    # Count python processes running `anthos gateway run`. If the
    ...
    assert n <= 1, (
        f"expected at most one supervised python `anthos gateway run` "
