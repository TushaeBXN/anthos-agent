...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    Use `anthos config set` which writes config.yaml under HERMES_HOME.
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    for it by writing the same file shape via `anthos config set`-style
    ...
    Specifically: pretend the operator ran `anthos login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the hermes UID. We use `anthos auth list` since that touches the
    ...
    # `anthos auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `anthos config set` first to
