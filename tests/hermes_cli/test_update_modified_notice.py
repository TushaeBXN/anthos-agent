"""Guard: every `anthos update` path that reports user-modified skills must
also tell the user how to find them.

`anthos update` keeps (does not overwrite) bundled skills the user edited and
...
at ``anthos skills list-modified`` so the count is actionable — otherwise,
...
def test_every_user_modified_notice_points_at_list_modified():
    ...
    for idx in count_sites:
        ...
        assert _HINT_RE.search(window), (
            ...
            "`anthos skills list-modified` within the following lines — the "
