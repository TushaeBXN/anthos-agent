...
`anthos gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`anthos -p typo gateway start` should print a clear message and
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `anthos gateway run --replace`, the
