"""Regression test: ``anthos mcp add --command`` must not clobber the
...
result, ``anthos mcp add foo --url ...`` silently launches chat instead
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`anthos mcp add foo --url ...` must keep args.command == "mcp".
