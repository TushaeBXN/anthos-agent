...
# spawned with `anthos -p <profile>` must read/write the same kanban.db
...
class TestSharedBoardPaths:
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # Worker's perspective (profile activated by `anthos -p coder`).
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'anthos kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    assert "Anthos Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
