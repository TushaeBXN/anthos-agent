...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    the dispatch path inert on Fly, so `anthos gateway restart` spawned a
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``anthos gateway run``, an agent action, the Open WebUI helper) holds
    ...
    # Root profile: bare `anthos gateway run --replace` (no -p flag).
...
#
# `anthos gateway stop` inside a container dispatches through
