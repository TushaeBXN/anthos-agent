"""Tests for ``anthos dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``anthos update``
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        # for the post-`anthos update` path).
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``anthos dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``anthos dashboard --stop`` / ``--status`` actually parse."""
