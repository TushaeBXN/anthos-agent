...
This saves 500-650ms on ``anthos --help``, ``anthos version``,
``anthos logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``anthos --help`` in-process and parse the subcommand block.
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``anthos help <cmd>`` skips discovery; it won't show
