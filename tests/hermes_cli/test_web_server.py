...
class TestWebServerEndpoints:
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `anthos model` but
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `anthos import` doesn't auto-abort at the overwrite prompt."""
        import hermes_cli.web_server as ws
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Anthos Agent"}
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``anthos gateway restart`` child (e.g. when a stale
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``anthos model`` custom flow. Regression for the desktop loop where a
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `anthos model`), not the first curated paid entry."""
...
# or ``sh -c 'printf …'``) instead of the real ``anthos --tui`` binary.  The
