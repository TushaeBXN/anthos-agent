"""Tests for `anthos memory setup [provider]` routing.
...
``anthos memory setup honcho``) without the interactive picker — which matters
because the per-provider ``anthos <provider>`` subcommand is only registered
