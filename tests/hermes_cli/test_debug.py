"""Tests for ``anthos debug`` CLI command and debug utilities."""
...
class TestScheduleAutoDelete:
    ...
    handle the DELETE requests synchronously on the next ``anthos debug``
    invocation.
    """
...
class TestBuildDebugShare:
    ...
    Backs both ``anthos debug share`` (CLI) and ``POST /api/ops/debug-share``
