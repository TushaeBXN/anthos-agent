"""Tests for the stale-dashboard handling run at the end of ``anthos update``.

``anthos update`` detects ``anthos dashboard`` processes left over from the
...
class TestWindowsWmicEncoding:
    ...
    `anthos update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_returns_none_stdout_does_not_crash(self, monkeypatch):
        ...
        ``None.split('\\n')`` and aborting `anthos update` (#17049)."""
        monkeypatch.setattr(sys, "platform", "win32")
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
