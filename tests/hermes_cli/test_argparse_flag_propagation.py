...
subparser, placing the flag BEFORE the subcommand (e.g. 'anthos --yolo chat')
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `anthos gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `anthos <argv>` must exit 0 (help) rather than
