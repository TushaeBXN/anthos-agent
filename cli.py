#!/usr/bin/env python3
"""
Anthos Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Anthos Agent, inspired by Claude Code.
...
try:
    ...
    # yet — happens during partial ``anthos update`` where git-reset landed
...
def load_cli_config() -> Dict[str, Any]:
    ...
    If HERMES_IGNORE_USER_CONFIG=1 is set (via ``anthos chat --ignore-user-config``),
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Anthos, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Anthos - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
    ...
    # `anthos config`/`doctor`/guards (which use load_config) honor it. The
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``anthos --help`` paths inside the chat command flow).  See
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.hermes/hermes-agent`` clone is updated only by ``anthos update``,
...
def _prune_orphaned_branches(repo_root: str) -> None:
    ...
    These are auto-generated by ``anthos -w`` sessions and PR review
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "ANTHOS - AI Agent Framework"
        tiny_line = "ANTHOS"
    else:
        agent_name = _skin.get_branding("agent_name", "Anthos Agent") if _skin else "Anthos Agent"
    ...
    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        ...
        version_line = f"Anthos Agent v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Brian Tushae Thomas[/]\n"
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Anthos Agent.
    ...
    def __init__(
        ...
        # by `anthos chat --ignore-rules` in hermes_cli/main.py. When true we
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `anthos update` to rebuild the venv) to restore persistence."
                )
            except Exception:
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from hermes_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "✦ Anthos")
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``anthos doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``anthos doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Anthos needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        if is_nous_hermes_non_agentic(model_name):
            ...
            self._console_print(
                ...
                "designed for use with Anthos Agent.[/]"
            )
            self._console_print(
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'anthos setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Anthos!{_RST}")
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``anthos sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `anthos sessions list` (gemini-cli#27770 port).
    ...
    def save_conversation(self):
        ...
        DB, so the live session remains resumable via ``anthos --resume <id>``
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Anthos session."),
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `anthos model` and gateway pickers).
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in {"quit", "exit"}:
            ...
            try:
                # Discover from disk (bundled + user), matching `anthos plugins
                ...
                # catalog lives behind `anthos plugins list`.
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("  Run `anthos portal` to log in, then /credits.")
            return
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `anthos portal` to log in, then /billing.")
            return
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `anthos portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Anthos Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Anthos Agent! Type your message or /help for commands."
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from hermes_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Anthos Agent")
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Anthos needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Anthos needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Anthos needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `anthos sessions list`
...
def main(
    ...
    """
    Anthos Agent CLI - Interactive AI Assistant
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``anthos chat -q "work kanban task <id>"``;
            ...
            if quiet:
                ...
                # Single-query mode (`anthos chat -q "…"`): skip the welcome
